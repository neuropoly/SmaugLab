...
PIPELINE_ORDER: Mapping[Backend, tuple[str, ...]] = MappingProxyType(
    {
        ...
        # reached through a `kernel_type` / `func` / `invert_image` argument are expanded
        # into the leaf classes that replaced them, in the position the argument was read.
        Backend.GPU: (
            ...
            "RandomRedistributeSegGPU",  # RedistributeSegTransform
            "RandomScharrGPU",  # ScharrTransform
            # No ladder key: the GPU Laplace kernel was reachable only by setting
            # kernel_type="Laplace" on ScharrTransform. Placed next to its sibling.
            "RandomLaplaceGPU",
            ...
            "RandomBrightnessGPU",  # BrightnessTransform
            "RandomGammaGPU",  # GammaTransform
            "RandomInvGammaGPU",  # InvGammaTransform
            "RandomContrastGPU",  # ContrastTransform
            # FunctionTransform built one transform per entry in its `func` list, in this
            # order; the leaves keep it.
            "RandomLog1pGPU",
            "RandomSqrtGPU",
            "RandomSinGPU",
            "RandomExpGPU",
            "RandomSigmoidGPU",
            "RandomLowResTransformGPU",  # SimulateLowResTransform
            "RandomAcqTransformGPU",  # AcqTransform
            "RandomCropTransformGPU",  # CropTransform
            "RandomBiasFieldGPU",  # BiasFieldTransform
            "ZscoreNormalizationGPU",  # ZscoreNormalizationTransform
        ),
        # From the ladder in cpu/transforms.py, top to bottom.
        Backend.CPU: (
...
def pipeline_position(entry: AugEntry) -> int:
    """Index of `entry` in its backend's pipeline order.

    Unlisted names sort last rather than raising: only `isolated()` can produce one,
    and a test's throwaway class should not make `entries()` explode.
    """
    order = PIPELINE_ORDER[entry.backend]
    return order.index(entry.name) if entry.name in order else len(order)
...
# a `global` statement.
_state: dict[str, bool] = {"loaded": False, "check_position": True}
...
def register_entry(entry: AugEntry) -> AugEntry:
    ...
    if _state["check_position"] and entry.name not in PIPELINE_ORDER[entry.backend]:
        raise RegistryError(
            f"{entry.backend.value} augmentation {entry.name!r} is not in PIPELINE_ORDER, so the pipeline "
            f"has nowhere to put it. Add it to the tuple in registry.py, at the position it should run."
        )
...
def entries(
    ...
    return sorted(found, key=lambda e: (e.backend.value, pipeline_position(e)))
...
def isolated(order: Mapping[Backend, tuple[str, ...]] | None = None) -> Iterator[None]:
    ...
    `order` replaces PIPELINE_ORDER for the block, and registration is checked against
    it. Omitting it disables the position check for the block instead, which is what
    most tests want: they register throwaway classes whose pipeline position is not the
    thing under test. `test_registering_a_class_absent_from_pipeline_order_fails`
    passes an explicit table to cover the check itself.
    """
    global PIPELINE_ORDER  # noqa: PLW0603 -- swapping the table is the point
    saved = {backend: dict(entries_) for backend, entries_ in _REGISTRY.items()}
    saved_order = PIPELINE_ORDER
    was_loaded = _state["loaded"]
    clear(mark_loaded=True)
    saved_check = _state["check_position"]
    if order is None:
        _state["check_position"] = False
    else:
        # Explicitly back on: these blocks nest inside an outer isolated() that turned
        # the check off, and the point of passing a table is to be checked against it.
        _state["check_position"] = True
        PIPELINE_ORDER = MappingProxyType({backend: tuple(order.get(backend, ())) for backend in Backend})
    try:
        yield
    finally:
        _state["check_position"] = saved_check
        PIPELINE_ORDER = saved_order
