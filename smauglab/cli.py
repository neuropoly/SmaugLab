...
def cmd_list(args) -> int:
    ...
    for entry in entries:
        ...
        print(f"  {entry.order:>4}  {entry.name:<{width}}  {entry.group.value:<4} {entry.summary}")
    return 0
...
def cmd_show(args) -> int:
    ...
    print(f"{entry.name}  ({entry.backend.value}, group {entry.group.value}, order {entry.order}, aug_id {entry.aug_id.value})")
    if entry.summary:
