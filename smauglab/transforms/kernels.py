...
not equivalent, and the divergence hid two real bugs:

* `gpu/contrast.py::get_gaussian_kernel1d` sampled the Gaussian at `arange(k)` instead
  of a centred range, so its peak sat at index 0. The resulting 3-D kernel had its
  maximum at corner `[0,0,0]`, which meant `RandomGaussianBlurGPU` and
  `RandomUnsharpMaskGPU` blurred *and* translated the image by about a voxel --
  relative to a segmentation mask that was not translated. The three sibling
  implementations were all centred correctly.
* The 2-D Scharr x-kernel in `cpu/contrast.py` had `[-10, 0, -10]` as its middle row
  instead of `[-10, 0, 10]`. It summed to -20, so it was not a gradient operator at
  all. The 3-D CPU kernel and both GPU kernels were right; only the copy nobody
  compared against the others was wrong.

Both are fixed here. Anything that convolves or blurs should import from this module
rather than growing a fifth copy.

Behaviour notes for the merge:

* The separable blur uses `radius = ceil(3*sigma)`. `domain_transfer` and `fromSeg`
  used `round(3*sigma)`, which is never wider, so kernels may now be one tap larger.
* Padding is `reflect` everywhere. `domain_transfer` used `replicate` and `fromSeg`
  used conv2d/3d's implicit zero padding; zero padding darkens the volume border,
  which is the one difference here that was visibly wrong rather than merely different.
"""
