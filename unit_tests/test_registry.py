...
class TestRegistration(RegistryTestCase):
    def test_decorator_registers_and_returns_the_class(self):
        ...
        decorated = registry.register(aug_id=AugId.SCHARR, backend=Backend.GPU, group=AugType.TA, order=10)(cls)
    ...
    def test_summary_defaults_to_the_first_docstring_line(self):
        cls = make_transform("RandomThingGPU", p=1.0)
        registry.register(aug_id=AugId.SCHARR, backend=Backend.GPU, group=AugType.TA, order=10)(cls)
    ...
    def test_duplicate_name_is_rejected(self):
        registry.register(aug_id=AugId.SCHARR, backend=Backend.GPU, group=AugType.TA, order=10)(make_transform("RandomThingGPU", p=1.0))
        with self.assertRaises(RegistryError) as caught:
            registry.register(aug_id=AugId.LAPLACE, backend=Backend.GPU, group=AugType.TA, order=20)(
                make_transform("RandomThingGPU", p=1.0)
            )
        self.assertIn("already registered", str(caught.exception))

    def test_duplicate_order_within_a_backend_is_rejected(self):
        """Two transforms at the same order would have an ambiguous pipeline position."""
        registry.register(aug_id=AugId.SCHARR, backend=Backend.GPU, group=AugType.TA, order=10)(make_transform("RandomOneGPU", p=1.0))
        with self.assertRaises(RegistryError) as caught:
            registry.register(aug_id=AugId.LAPLACE, backend=Backend.GPU, group=AugType.TA, order=10)(make_transform("RandomTwoGPU", p=1.0))
        self.assertIn("order 10 is taken", str(caught.exception))

    def test_same_order_on_different_backends_is_fine(self):
        registry.register(aug_id=AugId.SCHARR, backend=Backend.GPU, group=AugType.TA, order=10)(make_transform("RandomScharrGPU", p=1.0))
        registry.register(aug_id=AugId.SCHARR, backend=Backend.CPU, group=AugType.TA, order=10)(make_transform("ScharrConvTransform"))
    ...
    def test_var_keyword_without_forwards_to_is_rejected(self):
        ...
        with self.assertRaises(RegistryError) as caught:
            registry.register(aug_id=AugId.SCHARR, backend=Backend.GPU, group=AugType.TA, order=10)(RandomSloppyGPU)
    ...
    def test_var_keyword_is_allowed_when_forwards_to_is_declared(self):
        ...
        registry.register(aug_id=AugId.SYNTHSEG, backend=Backend.GPU, group=AugType.TA, order=10, forwards_to=target)(RandomForwardingGPU)
...
class TestLookup(RegistryTestCase):
    def setUp(self) -> None:
        super().setUp()
        registry.register(aug_id=AugId.SCHARR, backend=Backend.GPU, group=AugType.TA, order=20)(
            make_transform("RandomScharrGPU", p=1.0, absolute=True)
        )
        registry.register(aug_id=AugId.FLIP, backend=Backend.GPU, group=AugType.GEO, order=10)(
            make_transform("RandomFlipTransformGPU", p=1.0)
        )
...
class TestAcceptedParams(RegistryTestCase):
    def test_params_come_from_the_constructor_signature(self):
        registry.register(aug_id=AugId.GAUSSIAN_NOISE, backend=Backend.GPU, group=AugType.GE, order=10)(
    ...
    def test_cpu_wrapped_transforms_accept_p_even_though_the_class_does_not(self):
        ...
        registry.register(aug_id=AugId.SCHARR, backend=Backend.CPU, group=AugType.TA, order=10)(
            make_transform("ScharrConvTransform", absolute=True)
        )
    ...
    def test_unwrapped_cpu_transforms_do_not_get_a_synthetic_p(self):
        registry.register(aug_id=AugId.MIRROR, backend=Backend.CPU, group=AugType.GEO, order=10, wrap_random=False)(
    ...
    def test_required_params_are_those_without_a_default(self):
        ...
        registry.register(aug_id=AugId.AFFINE, backend=Backend.GPU, group=AugType.GEO, order=10)(RandomAffineGPU)
