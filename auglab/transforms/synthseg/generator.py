...
class SynthSegGenerator(nn.Module):
    ...
    def __init__(
        self,
        generation_labels: Sequence[int] | None = None,
        output_labels: Sequence[int] | None = None,
    ...
    # ------------------------------------------------------------------
    @torch.no_grad()
    def forward(self, label_map: torch.Tensor, image: torch.Tensor | None = None) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.em_label_completion:
            if image is None:
                if not self._warned_em:
                    print(
                        f"{type(self).__name__}: em_label_completion is enabled but no "
                        f"image was provided; falling back to plain generation.",
                        flush=True,
                    )
        ...
        if self.flipping and float(torch.rand((), device=device)) < 0.5:
            label_values_flip = (
                torch.as_tensor(gen_labels, dtype=torch.long, device=device) if gen_labels is not None else FN.infer_label_values(labels)
            )
        ...
        if self.apply_intensity_augmentation:
            synth = FN.intensity_augmentation(synth, clip=self.clip, gamma_std=self.gamma_std, normalise=self.normalise)
    ...
    def _simulate_resolution(self, image: torch.Tensor) -> torch.Tensor:
        ...
        for c in range(image.shape[1]):
            ...
            if self.randomise_res:
                res, thickness = FN.sample_resolution(atlas_res, self.max_res_iso, self.max_res_aniso)
            else:
...
if __name__ == "__main__":
    ...
    print("output image :", tuple(image.shape), "range", (round(float(image.min()), 3), round(float(image.max()), 3)))
